...
def process_pbp_metadata(lazy_pbp: pl.LazyFrame) -> pl.DataFrame:
    """Computes rest days."""
    games = lazy_pbp.select(["game_id", "game_date", "home_team", "away_team"]).unique().collect()
...
def main():
    ...
    seasons = sorted(lazy_pbp.select(pl.col("season")).unique().collect()["season"].to_list())
    ...
    for season in track(seasons, description="Processing play-by-play data for RAPM regressions..."):
        ...
        for session in ["R", "P"]:
            ...
            season_pbp = lazy_pbp.filter(pl.col("season") == season, pl.col("session") == session).collect()
